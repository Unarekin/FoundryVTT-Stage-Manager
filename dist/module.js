...
(() => {
  ...
  // node_modules/mini-rx-store/index.esm.js
  var index_esm_exports = {};
  __export(index_esm_exports, {
    Actions: () => Actions,
    ComponentStore: () => ComponentStore,
    FeatureStore: () => FeatureStore,
    ImmutableStateExtension: () => ImmutableStateExtension,
    ...
    actions$: () => actions$,
    configureComponentStores: () => configureComponentStores,
    configureStore: () => configureStore,
    createComponentStateSelector: () => createComponentStateSelector,
    createComponentStore: () => createComponentStore,
    createEffect: () => createEffect,
    createFeatureSelector: () => createFeatureSelector,
    createFeatureStateSelector: () => createFeatureStateSelector,
    createFeatureStore: () => createFeatureStore,
    createSelector: () => createSelector,
    hasEffectMetaData: () => hasEffectMetaData,
    mapResponse: () => mapResponse,
    ofType: () => ofType,
    tapResponse: () => tapResponse,
    undo: () => undo
  });
  var StoreExtension = class {
    constructor() {
      this.sortOrder = 0;
    }
  };
  ...
  var EFFECT_METADATA_KEY = "@mini-rx/effectMetaData";
  var miniRxNameSpace = "@mini-rx";
  function createMiniRxActionType(miniRxActionType, featureKey) {
    return miniRxNameSpace + (featureKey ? "/" + featureKey : "") + "/" + miniRxActionType;
  }
  function createMiniRxAction(miniRxActionType, featureKey) {
    return {
      type: createMiniRxActionType(miniRxActionType, featureKey)
    };
  }
  var setStateActionTypeKey = "setStateActionType";
  function isFeatureStoreSetStateAction(action) {
    return action[setStateActionTypeKey] === "@mini-rx/feature-store";
  }
  function isComponentStoreSetStateAction(action) {
    return action[setStateActionTypeKey] === "@mini-rx/component-store";
  }
  var UNDO_ACTION = miniRxNameSpace + "/undo";
  ...
  function ofType(...allowedTypes) {
    return filter((action) => allowedTypes.some((type) => {
  ...
  function miniRxConsoleError(message, err) {
    console.error(miniRxNameSpace + ": " + message + "\nDetails:", err);
  }
  function hasEffectMetaData(param) {
    return param.hasOwnProperty(EFFECT_METADATA_KEY);
  }
  function beautifyActionForLogging(action, state) {
  ...
  function calcNewState(state, stateOrCallback) {
    const newPartialState = typeof stateOrCallback === "function" ? stateOrCallback(state) : stateOrCallback;
    return Object.assign(Object.assign({}, state), newPartialState);
  }
  function combineMetaReducers(metaReducers) {
    return (reducer) => {
      return metaReducers.reduceRight((previousValue, currentValue) => {
  ...
  function defaultEffectsErrorHandler(observable$, retryAttemptLeft = 10) {
    return observable$.pipe(catchError((error2) => {
      miniRxConsoleError(`An error occurred in the Effect. MiniRx resubscribed the Effect automatically and will do so ${retryAttemptLeft - 1} more times.
Please provide error handling inside the Effect using \`catchError\` or \`tapResponse\`.`, error2);
  ...
  var State = class {
    constructor(initialState) {
      ...
      this.stateSource = new BehaviorSubject(this.initialState);
      this.state$ = this.stateSource.asObservable().pipe(
  ...
  var ActionsOnQueue = class {
    constructor() {
      this.actionsSource = new Subject();
      this.actions$ = this.actionsSource.asObservable().pipe(
  ...
  var hasUndoExtension = false;
  var isStoreInitialized = false;
  var reducerState = new State();
  var reducer$ = reducerState.select((v) => {
    const combinedMetaReducer = combineMetaReducers(v.metaReducers);
    const combinedReducer = v.combineReducersFn(v.featureReducers);
    return combinedMetaReducer(combinedReducer);
  });
  function hasFeatureReducers() {
    return !!Object.keys(reducerState.get().featureReducers).length;
  }
  function checkFeatureExists(featureKey) {
    if (reducerState.get().featureReducers.hasOwnProperty(featureKey)) {
      miniRxError(`Feature "${featureKey}" already exists.`);
    }
  }
  function addReducer(featureKey, reducer) {
    initStore();
    checkFeatureExists(featureKey);
    reducerState.patch((state) => ({
      featureReducers: Object.assign(Object.assign({}, state.featureReducers), {
        [featureKey]: reducer
      })
    }));
  }
  function removeReducer(featureKey) {
    reducerState.patch((state) => ({
      featureReducers: omit(state.featureReducers, featureKey)
    }));
  }
  function addMetaReducers(...reducers) {
    reducerState.patch((state) => ({
      metaReducers: [...state.metaReducers, ...reducers]
    }));
  }
  function updateCombineReducersFn(combineReducersFn) {
    reducerState.patch({
      combineReducersFn
    });
  }
  var actionsOnQueue = new ActionsOnQueue();
  var actions$ = actionsOnQueue.actions$;
  ...
  function configureStore$1(config2 = {}) {
    var _a, _b;
    initStore();
    if (hasFeatureReducers()) {
      miniRxError("`configureStore` detected reducers. Did you instantiate FeatureStores before calling `configureStore`?");
    }
    if (config2.combineReducersFn) {
      updateCombineReducersFn(config2.combineReducersFn);
    }
    if ((_a = config2.metaReducers) === null || _a === void 0 ? void 0 : _a.length) {
      addMetaReducers(...config2.metaReducers);
    }
    if ((_b = config2.extensions) === null || _b === void 0 ? void 0 : _b.length) {
      const sortedExtensions = sortExtensions(config2.extensions);
      sortedExtensions.forEach((extension) => addExtension(extension));
    }
    if (config2.reducers) {
      Object.keys(config2.reducers).forEach((featureKey) => {
        addReducer(featureKey, config2.reducers[featureKey]);
      });
    }
    if (config2.initialState) {
      appState.set(config2.initialState);
    }
    dispatch(createMiniRxAction(
      "init"
      /* INIT */
    ));
  }
  function addFeature(featureKey, reducer, config2 = {}) {
    var _a;
    reducer = ((_a = config2.metaReducers) === null || _a === void 0 ? void 0 : _a.length) ? combineMetaReducers(config2.metaReducers)(reducer) : reducer;
    if (typeof config2.initialState !== "undefined") {
      reducer = createReducerWithInitialState(reducer, config2.initialState);
    }
  ...
  function removeFeature(featureKey) {
  ...
  function effect(effect$) {
    ...
    effectWithErrorHandler$.subscribe((action) => {
      ...
      if (hasEffectMetaData(effect$)) {
        const metaData = effect$[EFFECT_METADATA_KEY];
  ...
  function addExtension(extension) {
    const metaReducer = extension.init();
    if (metaReducer) {
  ...
  function createReducerWithInitialState(reducer, initialState) {
    return (state = initialState, action) => {
      return reducer(state, action);
    };
  }
  function omit(object, keyToOmit) {
    return Object.keys(object).filter((key) => key !== keyToOmit).reduce((prevValue, key) => {
      prevValue[key] = object[key];
      return prevValue;
    }, {});
  }
  var storeCore = /* @__PURE__ */ Object.freeze({
    __proto__: null,
    get hasUndoExtension() {
  ...
  var isStoreConfigured = false;
  function configureStore(config2) {
    if (!isStoreConfigured) {
      configureStore$1(config2);
  ...
  var BaseStore = class {
    constructor() {
      ...
      this.state$ = this._state.select();
      this.isStateInitialized = false;
      this.notInitializedErrorMessage = `${this.constructor.name} has no initialState yet. Please provide an initialState before updating/getting state.`;
      this.initializedErrorMessage = `${this.constructor.name} has initialState already.`;
      this.select = this._state.select.bind(this._state);
    }
    get state() {
    ...
    setState(stateOrCallback, name) {
      const dispatchFn = (stateOrCallback2, name2) => {
        this.assertStateIsInitialized();
        return this._dispatchSetStateAction(stateOrCallback2, name2);
      };
      return isObservable(stateOrCallback) ? this._sub.add(stateOrCallback.subscribe((v) => dispatchFn(v, name))) : dispatchFn(stateOrCallback, name);
    }
    effect(effectFn) {
      ...
      return (observableOrValue) => {
        isObservable(observableOrValue) ? this._sub.add(observableOrValue.subscribe((v) => subject.next(v))) : subject.next(observableOrValue);
      };
    }
  ...
  var FeatureStore = class extends BaseStore {
    constructor(featureKey, initialState, config2 = {}) {
      super();
      this.featureId = generateId();
      this._featureKey = config2.multi ? featureKey + "-" + generateId() : featureKey;
      if (initialState) {
    ...
    setInitialState(initialState) {
      ...
      addFeature(this._featureKey, createFeatureStoreReducer(this.featureId, initialState));
      this._sub.add(appState.select((state) => state[this.featureKey]).subscribe((v) => this._state.set(v)));
    }
    /** @internal
     * Implementation of abstract method from BaseStore
     */
    _dispatchSetStateAction(stateOrCallback, name) {
      const action = createSetStateAction$1(stateOrCallback, this.featureId, this.featureKey, name);
      dispatch(action);
      return action;
    }
    // Implementation of abstract method from BaseStore
    undo(action) {
      hasUndoExtension ? dispatch(undo(action)) : miniRxError("UndoExtension is not initialized.");
    }
    destroy() {
      super.destroy();
  ...
  function createFeatureStoreReducer(featureId, initialState) {
    return (state = initialState, action) => {
      if (isFeatureStoreSetStateAction(action) && action.featureId === featureId) {
        return calcNewState(state, action.stateOrCallback);
      }
      return state;
    };
  }
  function createSetStateAction$1(stateOrCallback, featureId, featureKey, name) {
    const miniRxActionType = "set-state";
    return {
  ...
  function createFeatureStore(featureKey, initialState, config2 = {}) {
    return new FeatureStore(featureKey, initialState, config2);
  }
  var componentStoreConfig = void 0;
  function configureComponentStores(config2) {
    if (!componentStoreConfig) {
      componentStoreConfig = config2;
      return;
    }
    miniRxError("`configureComponentStores` was called multiple times.");
  }
  var csFeatureKey = "component-store";
  var ComponentStore = class extends BaseStore {
    constructor(initialState, config2) {
      super();
      this.actionsOnQueue = new ActionsOnQueue();
      this.hasUndoExtension = false;
      this.extensions = [];
      const metaReducers = [];
      if (config2 === null || config2 === void 0 ? void 0 : config2.extensions) {
        if (config2.extensions && (componentStoreConfig === null || componentStoreConfig === void 0 ? void 0 : componentStoreConfig.extensions)) {
          this.extensions = mergeExtensions(componentStoreConfig.extensions, config2.extensions);
        } else {
          this.extensions = config2.extensions;
        }
      } else if (componentStoreConfig === null || componentStoreConfig === void 0 ? void 0 : componentStoreConfig.extensions) {
      ...
      sortExtensions(this.extensions).forEach((ext) => {
        ...
        metaReducers.push(ext.init());
        if (ext.id === 1) {
          this.hasUndoExtension = true;
        }
      });
      this.combinedMetaReducer = combineMetaReducers(metaReducers);
      this._sub.add(this.actionsOnQueue.actions$.subscribe((action) => {
    ...
    setInitialState(initialState) {
      ...
      this.dispatch(createMiniRxAction(
        "init",
        csFeatureKey
      ));
    }
     ...
     */
    _dispatchSetStateAction(stateOrCallback, name) {
      const action = createSetStateAction(stateOrCallback, name);
    ...
    // Implementation of abstract method from BaseStore
    undo(action) {
      this.hasUndoExtension ? this.dispatch(undo(action)) : miniRxError(`${this.constructor.name} has no UndoExtension yet.`);
    }
    destroy() {
      if (this.reducer) {
        this.dispatch(createMiniRxAction(
  ...
  function createSetStateAction(stateOrCallback, name) {
    const miniRxActionType = "set-state";
    return {
  ...
  function createComponentStoreReducer(initialState) {
    return (state = initialState, action) => {
      if (isComponentStoreSetStateAction(action)) {
        return calcNewState(state, action.stateOrCallback);
      }
      return state;
    };
  }
  function mergeExtensions(global, local) {
    const extensions = [];
    ...
    global.forEach((globalExt) => {
      local.forEach((localExt) => {
        if (localExt.id === globalExt.id) {
          extensions.push(localExt);
          localCopy = localCopy.filter((item) => item.id !== localExt.id);
          globalCopy = globalCopy.filter((item) => item.id !== globalExt.id);
        }
      });
    });
    return [
  ...
  function createComponentStore(initialState, config2) {
    return new ComponentStore(initialState, config2);
  }
  function isEqual(first2, second) {
    if (first2 === second) {
      return true;
    }
    if (Number.isNaN(first2) && Number.isNaN(second)) {
      return true;
    }
    return false;
  }
  ...
  var createFeatureStateSelector = createFeatureSelector;
  ...
  function extractArgsFromSelectorsDictionary(selectorsDictionary) {
    ...
    const projector = (...selectorResults) => resultKeys.reduce((result, key, index) => Object.assign(Object.assign({}, result), {
  ...
  var ReduxDevtoolsExtension = class extends StoreExtension {
    constructor(options) {
      super();
      ...
      this.devtoolsExtension = window.__REDUX_DEVTOOLS_EXTENSION__;
      this.options = Object.assign(Object.assign({}, defaultOptions), this.options);
    }
    get optionsForNgExtension() {
      return this._optionsForNgExtension;
    }
    init() {
      if (this.devtoolsExtension) {
        this.devtoolsConnection = this.devtoolsExtension.connect(this.options);
        actions$.pipe(withLatestFrom(appState.select()), tap(([action, state]) => {
          const actionForDevTools = beautifyActionForLogging(action);
  ...
  var LoggerExtension = class extends StoreExtension {
    constructor() {
      super(...arguments);
      this.id = 2;
      this.hasCsSupport = true;
    }
    init() {
  ...
  function loggerMetaReducer(reducer) {
    return (state, action) => {
      const actionToLog = beautifyActionForLogging(action);
      const nextState = reducer(state, action);
      console.log("%c" + action.type, "color: #25c2a0", "\nAction:", actionToLog, "\nState: ", nextState);
      return nextState;
    };
  }
  ...
  var ImmutableStateExtension = class extends StoreExtension {
    constructor() {
      super(...arguments);
      this.id = 0;
      this.hasCsSupport = true;
  ...
  var UndoExtension = class extends StoreExtension {
    constructor(config2 = {
  ...
  function createUndoMetaReducer(bufferSize) {
    let executedActions = [];
    let initialState;
    return (rootReducer) => {
      return (state, action) => {
  ...
  function tapResponse(fn1OrObject, fn2, fn3) {
    return (source) => {
      const tapResponseObj = typeof fn1OrObject === "function" ? {
        next: fn1OrObject,
        error: fn2,
        finalize: fn3
      } : fn1OrObject;
      return source.pipe(
        ...
        catchError(() => EMPTY),
        tapResponseObj.finalize ? finalize(() => {
          tryCatch(() => tapResponseObj.finalize(), "finalize");
        }) : identity
        // Conditionally apply operator: https://rxjs.dev/api/index/function/identity
      );
    };
  }
  function tryCatch(fn, whichCallback) {
    try {
      fn();
    } catch (error2) {
      miniRxConsoleError(`An error occurred in the \`tapResponse\` ${whichCallback} callback.`, error2);
    }
  }
  function mapResponse(mapFn, errorFn) {
    return (source) => source.pipe(map((v) => mapFn(v)), mergeMap((mapFnResult) => {
      return Array.isArray(mapFnResult) ? from(mapFnResult) : of(mapFnResult);
    }), catchError((err) => {
      const errorFnResult = errorFn(err);
      return errorFnResult ? Array.isArray(errorFnResult) ? from(errorFnResult) : of(errorFnResult) : EMPTY;
    }));
  }
  var DEFAULT_EFFECT_CONFIG = {
    dispatch: true
  };
  function createEffect(v, config2) {
    const value = Object.assign(Object.assign({}, DEFAULT_EFFECT_CONFIG), config2);
